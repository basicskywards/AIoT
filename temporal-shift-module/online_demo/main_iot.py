    ...
    while True:
        ...
        try:
            ...
            if ODF_data != None:
                ...
                if int(ODF_data[0]/255.0) == 1 and on_flag == 1:
                    print('\niottalk --> AC ON!!!')
                    obj.left()
                    time.sleep(0.5)
                    on_flag = 0 # enable OFF
 
                elif int(ODF_data[0]) == 0 and on_flag == 0:
                    print('\niottalk --> AC OFF')
                    obj.left()
                    time.sleep(0.5)
                    on_flag = 1 # enable ON
        ...
        ## Control servo by hand gestures
        if idx == 20: # thumb up
            cnt_thumb_up += 1
        ...
        if idx == 20 and on_flag == 0 and cnt_thumb_up == 1:
            print('\nThumb Up --> AC ON!!!')
            obj.left()
            time.sleep(5)
            on_flag = 1 # enable OFF
            cnt_stop_sign = 0 # enable OFF

        if idx == 14 and on_flag == 1 and cnt_stop_sign == 1:
            print('\nStop Sign --> AC OFF')
            obj.left()
            time.sleep(5)
            on_flag = 0 # enable ON
            cnt_thumb_up = 0 # enable ON

        #time.sleep(0.002) # delay for iottalk
